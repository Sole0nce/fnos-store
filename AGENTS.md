...
├── web/                     # Embedded frontend build output — generated by Vite but TRACKED in git
...
- **DO NOT write to `web/` directly** — it's generated by `npm run build` and embedded at compile time.
  It IS tracked in git, so any frontend source change must be rebuilt and the regenerated `web/`
  committed alongside it — otherwise the embedded bundle silently lags the source it was built from.
